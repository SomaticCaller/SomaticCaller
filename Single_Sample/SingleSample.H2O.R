...
option_list<-list(make_option(c("--FeatureType"), type="character", default=NULL, 
                  ...
                  make_option(c("--algorithms"), type="character", default="glm,gbm,rf,dl,xgboost", 
                              ...
                              metavar="character"),
                  make_option(c("--sample_factors"), type="character", default=NULL, 
                              help="balance classes ratio,comma separated", metavar="character"),
                  ...
                  make_option(c("--colnames"), type="character", default="Detail_Group,Stage_TNM,Library",
                              help="colomn names, comma separated.", metavar="character"),
...
tryCatch({
    ...
    colnames(id.info.all)[c(1,2)] = c("Sample","Type")
    id.info = id.info.all[,c(1,2)]
    sample_split_cols = strsplit(opt$colnames, split = ",")[[1]]
    
    #id.info.all$SampleOri = gsub("-3X$", "", id.info.all$Sample)
    id.info.all$SampleOri = gsub("-.{1,3}X$", "", id.info.all$Sample)
    id.info.all$SampleOri = gsub("-raw$", "", id.info.all$SampleOri)
    id.info.all.ori = unique(id.info.all[,c("SampleOri","Type",sample_split_cols)])
    id.info.all.ori$split_factor = do.call(paste, c(id.info.all.ori[, sample_split_cols], sep = "-")) 
    set.seed(opt$seed)
    sample_train = id.info.all.ori %>% group_by(split_factor) %>% slice_sample(prop = opt$ratio)
    
    sample_train = id.info[id.info.all$SampleOri %in% sample_train$SampleOri,c("Sample","Type")]
    ...
    if (length(train_list) > 1) {
        for (i in 2:length(train_list)){
            combined_data_train = inner_join(combined_data_train, train_list[[i]][,setdiff(colnames(train_list[[i]]), "Type")], c("Sample" = "Sample"))
            combined_data_test = inner_join(combined_data_test, test_list[[i]][,setdiff(colnames(test_list[[i]]), "Type")], c("Sample" = "Sample"))
        }
    }
    
    combined_data_train$Type = as.factor(combined_data_train$Type)
    print(table(combined_data_train$Type))
    ...
    colnames(test_score_df) = c("SampleID","Type")
    
    sample_factors = as.numeric(strsplit(opt$sample_factors, split = ",")[[1]])
    ...
    for (feature_type in feature_type_list){
        ...
        print(h2o.table(as.h2o(tmp_train)['Type']))
        ...
        if ("glm" %in% algorithms_type_list){
            ...
            model_list[[feature_type]][["glm"]] <- h2o.glm(x = predictor, y = response, training_frame = train_list[[feature_type]], nfolds = nfolds, 
                                                           seed = 1234, fold_assignment = "Stratified", keep_cross_validation_predictions = TRUE,
        ...
        if ("gbm" %in% algorithms_type_list){
            ...
            model_list[[feature_type]][["gbm"]]<- h2o.gbm(x = predictor, y = response, training_frame = train_list[[feature_type]], nfolds = nfolds, 
                                                          ...
                                                          balance_classes = TRUE,class_sampling_factors = sample_factors)
        }
        ...
        # # svm: parameters were different
        # if ("svm" %in% algorithms_type_list){
        #   print("    svm ...")
        #   model_list[[feature_type]][["svm"]] = h2o.psvm(x = predictor, y = response, training_frame = train_list[[feature_type]],
        #                                                  seed = 1234,
        #                                                  model_id = paste0(feature_type,"_","svm_seed",opt$seed))
        # }
